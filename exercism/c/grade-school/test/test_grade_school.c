...
static void test_add_student_with_too_long_name(void)
{
   //TEST_IGNORE();
   roster_t input = {
      1, {
          (student_t) {2, "Blair-Jack-Adam-Smith-the-Third"}}
   };

   roster_t expected = {0};
   TEST_ASSERT_FALSE(
       add_student(input.students[0].name, input.students[0].grade))
   roster_t actual = get_roster();
   check_roster_names(&expected, &actual);
}
...
static void test_grade_no_matching_grade(void) {
   //TEST_IGNORE();
   roster_t input = {
      3, {
          (student_t) {5, "Franklin"},
...
static void test_grade_max_length(void) {
   //TEST_IGNORE();
   roster_t input = {
      20, {
          (student_t) {5, "Bradley"},
   ...
   check_roster_names(&input, &actual);
}
...
int main(void)
{
   ...
   RUN_TEST(test_grade_no_matching_grade);
   RUN_TEST(test_grade_no_matching_grade);
   RUN_TEST(test_grade_max_length);
   RUN_TEST(test_add_student_with_too_long_name);
